...
class PThread
{
	private:
		...
		PThread(){}
	public:
		PThread(int tm, threadpro autodelete, priority pride, const char *):m_pro(autodelete)
		{
...
Worker::Worker(
	...
	m_idleTimeout(idleTimeout*1000), m_closed(false), m_job(NULL), m_id(0),m_agent(agent)
{
...
Worker::~Worker()
{
	...
	if (m_job) {
		PJ_LOG(1, (__FILE__, "JOB\tDestroying Worker %d  with active Job %s ", m_id, (const char *)m_job->GetName()));
		delete m_job;
		m_job = NULL;
	}
	PJ_LOG(3, (__FILE__,"JOB\tWorker %d  destroyed", m_id));
}
...
Agent::~Agent()
{
	PJ_LOG(3,(__FILE__, "JOB\tDestroying active Workers for the Agent"));
	...
	PJ_LOG(3, (__FILE__, "JOB\tWorker threads to cleanup: %d total - %d busy, %d idle",(numBusyWorkers+numIdleWorkers),numBusyWorkers, numIdleWorkers));
	...
	PJ_LOG(3, (__FILE__, "JOB\tAgent and its Workers destroyed"));
}
...
void Agent::Exec(Job * job)
{
	...
	if (job) {
		...
		if (!m_active) {
			PJ_LOG(3, (__FILE__, "JOB\tAgent did not accept Job %s",(const char *)job->GetName()));
			delete job;
			job = NULL;
			return;
		}
	...
	if (destroyWorker) {
		PJ_LOG(3, (__FILE__, "JOB\tAgent did not accept Job %s" ,(job ? (const char *)job->GetName() : "<no job>")));
		worker->Destroy();
	}
}
...
Job::~Job()
{
	PJ_LOG(5, (__FILE__, "JOB\tJob %s deleted", (const char *)GetName()));
}
